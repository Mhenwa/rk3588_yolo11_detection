...
{
    if (mask == RKNN_NPU_CORE_0) return 0;
    if (mask == RKNN_NPU_CORE_1) return 1;
    if (mask == RKNN_NPU_CORE_2) return 2;
    return -1;
}
                      ...
                      WorkerPool* pool)
{
    ...
    for (int i = 0; i < thread_count; ++i) {
        ...
        if (core_idx >= 0) LOGI("Worker %d bind NPU core %d\n", i, core_idx);
        else LOGI("Worker %d bind NPU core auto\n", i);
